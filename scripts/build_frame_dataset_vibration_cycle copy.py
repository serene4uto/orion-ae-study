...
def convert_and_segment_dataset(
    ...
    for mat_path in tqdm(data_files, desc="Converting & Segmenting"):
        ...
        try:
            ...
            vibrometer_signal = mat['D'].squeeze()
            ...
            if len(cycles) < skip_cycles:
                logger.warning(f"Less than {skip_cycles} cycles found in {mat_path.name}")
                continue
            
            if len(cycles) < cycles_per_frame:
                logger.warning(f"Less than {cycles_per_frame} cycles found in {mat_path.name}")
                continue
            
            cycles_lengths = 
            logger.info(f"Cycle length average: {cycles_lengths_avg:.2f} samples ({cycles_lengths_avg * 1000 / SAMPLING_FREQUENCY_HZ:.2f} ms)")
            frame_length = cycles_lengths_avg * cycles_per_frame
            ...
            for cycle in cycles:
                for i in range(len(cycle)):
                    if i >= skip_cycles*cycles_lengths_avg:
                        frame_start_idx.append(cycle[i])
                    else:
                        continue
